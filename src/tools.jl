...
"""
function k_function_matrix_fast(covariance_matrix::Matrix{Float64})
    # Γ = cov + 1/2 I  (make a copy so we don't mutate input)
    Γ = copy(covariance_matrix)
    @inbounds for i in 1:size(Γ,1)
        Γ[i,i] += 0.5
    end
    ...
    K = zeros(ComplexF64, 2n, 2n)   # 32×32 for your case
...
export wick_out, W, compile_W_terms, W_fast, permutation_matrix, reorder, k_function_matrix, k_function_matrix_fast
